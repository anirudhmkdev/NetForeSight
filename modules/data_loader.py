...
FLOW_COLUMNS = [
    ...
    "tcp_flags", "packets", "bytes", "duration", "iat_mean",
    "iat_variance", "iat_max", "tcp_window", "has_fragment",
    "ttl", "retransmissions", "risk_score", "risk",
]
...
COLUMN_ALIASES = {
    ...
    "iat_variance": {"iat_variance", "iat_var", "flow_iat_std", "flow_iat_variance", "iat_std"},
    "iat_max": {"iat_max", "flow_iat_max", "max_iat"},
    "tcp_window": {"tcp_window", "window_size", "init_win_bytes", "win_bytes", "init_win_bytes_forward"},
    "has_fragment": {"has_fragment", "fragment", "frag", "fragmentation"},
    "ttl": {"ttl", "ip_ttl"},
...
def generate_demo_dataset(seed: int = 42) -> pd.DataFrame:
    ...
    for w, spec in enumerate(_window_spec()):
        ...
        df = pd.DataFrame({
            ...
            "duration": dur, "iat_mean": iat,
            "iat_variance": np.round(np.clip(iat * 0.1, 0, 1), 4),
            "iat_max": np.round(iat * 2.5, 3),
            "tcp_window": rng.choice([65535, 32768, 16384, 8192], n),
            "has_fragment": np.zeros(n, dtype=int),
            "ttl": ttl,
...
def _standardize(df: pd.DataFrame, dayfirst: bool = False,
    ...
    out["iat_variance"] = pd.to_numeric(_col("iat_variance", 0.0), errors="coerce").fillna(0.0).clip(lower=0.0)
    out["iat_max"] = pd.to_numeric(_col("iat_max", out["iat_mean"]), errors="coerce").fillna(out["iat_mean"]).clip(lower=0.0)
    out["tcp_window"] = pd.to_numeric(_col("tcp_window", 65535), errors="coerce").fillna(65535).astype(int).clip(lower=0)
    out["has_fragment"] = pd.to_numeric(_col("has_fragment", 0), errors="coerce").fillna(0).astype(int).clip(0, 1)
    ...
    try:
        ...
        for pkt in reader:
            ...
            if not pkt.haslayer(IP):
                continue
            ip = pkt[IP]
            frag = bool(pkt[IP].flags.MF or pkt[IP].frag > 0)
            if pkt.haslayer(TCP):
            ...
            if cell is None:
                flows[key] = {"ts": [ts], "bytes": [len(pkt)], "ttl": [int(ip.ttl)],
                              "flags": [flags], "seqs": [seq], "win_sizes": [win_size],
                              "frag_flags": [frag]}
            else:
                cell["ts"].append(ts)
                ...
                cell["win_sizes"].append(win_size)
                cell["frag_flags"].append(frag)
        reader.close()
    except Exception as exc:
        raise DataLoadError(
    ...
    for (src, dst, sport, dport, proto), cell in flows.items():
        ...
        for s in cell["seqs"]:
            if s in seen and s >= 0:
                retx += 1
            seen.add(s)
        iat_var = round(float(iats.var()) if len(iats) > 1 else 0.0, 4)
        iat_max = round(float(iats.max()) if len(iats) else float(iats.mean()) if len(iats) else 0.5, 3)
        win = int(np.median(cell["win_sizes"])) if cell.get("win_sizes") else 65535
        has_frag = int(any(cell.get("frag_flags", [])))
        rows.append({
