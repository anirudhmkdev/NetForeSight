...
class NetworkWindow:
    ...
    retrans: int
    risk: float  # 0-100
    avg_iat_var: float = 0.0
    avg_tcp_win: float = 0.0
...
def generate_windows(flows: pd.DataFrame, n_windows: int = 5) -> list[NetworkWindow]:
    ...
    for i in range(n_windows):
        ...
        iat_var_avg = float(sub["iat_variance"].mean()) if "iat_variance" in sub else 0.0
        tcp_win_avg = float(sub["tcp_window"].mean()) if "tcp_window" in sub else 65535.0
        vec = np.array([
            len(sub) / 5000.0,
            ...
            risk / 100.0,
            float(np.clip(iat_var_avg, 0, 1)),
            float(np.clip(tcp_win_avg / 65535.0, 0, 1)),
        ], dtype=float).round(4)
        windows.append(NetworkWindow(
            ...
            risk=risk,
            avg_iat_var=round(iat_var_avg, 4),
            avg_tcp_win=round(tcp_win_avg, 1),
            top_ports=top_ports,
            state_vector=vec,
        ))
