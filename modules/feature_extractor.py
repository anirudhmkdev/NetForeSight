...
def extract_features(flows: pd.DataFrame) -> pd.DataFrame:
    ...
    # Port scan signature: destination port diversity per source IP
    df["port_entropy"] = (
        df.groupby("src_ip")["dst_port"]
        .transform(lambda s: s.nunique() / max(len(s), 1))
        .clip(0, 1)
        .round(4)
    )
    df["bidi_ratio"] = (df["bytes"] / df["packets"].clip(lower=1)).clip(0, 9999).round(1)
...
def normalize_features(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for extra in ["iat_variance", "tcp_window", "port_entropy"]:
